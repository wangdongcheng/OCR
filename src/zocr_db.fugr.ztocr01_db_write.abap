FUNCTION ztocr01_db_write.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_INS) TYPE  ZTOCR01_T OPTIONAL
*"     REFERENCE(IT_UPD) TYPE  ZTOCR01_T OPTIONAL
*"     REFERENCE(IT_DEL) TYPE  ZTOCR01_T OPTIONAL
...
*-- delete
  IF it_del IS SUPPLIED AND it_del IS NOT INITIAL.
    DELETE ztocr01 FROM TABLE it_del.
...
*-- insert
  IF it_ins IS SUPPLIED AND it_ins IS NOT INITIAL.
    INSERT ztocr01 FROM TABLE it_ins.
    IF NOT syst-subrc IS INITIAL .
*--E1	009	Error updating table &
      MESSAGE a009(e1) WITH 'ZTOCR01' .                     "#EC NOTEXT
    ENDIF.
...
*-- update
  IF it_upd IS SUPPLIED AND it_upd IS NOT INITIAL.
    UPDATE ztocr01 FROM TABLE it_upd.
    IF NOT syst-subrc IS INITIAL .
*--E1	009	Error updating table &
      MESSAGE a009(e1) WITH 'ZTOCR01' .                     "#EC NOTEXT
    ENDIF.
